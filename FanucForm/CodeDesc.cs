...
namespace FanucSampling
{
    ...
    class StateCodeDesc
    {
        private static Dictionary<short, string> run;
        private static Dictionary<short, string> emergency;
        private static Dictionary<short, string> alarm;

        static StateCodeDesc()
        {
            // Status of automatic operation
            run = new Dictionary<short, string>();
            run.Add(0, "STOP"); // 机床没有运行程序
            run.Add(1, "HOLD"); // 程序展厅
            run.Add(2, "STaRT");// 程序开始运行
            run.Add(3, "MSTR(jog mdi)");
            run.Add(4, "ReSTaRt(not blinking) Except under manual mode and under cutter radius compensation outside corner");
            run.Add(5, "PRSR(program restart)");                       
            run.Add(6, "NSRC(sequence number search)");
            run.Add(7, "ReSTaRt(blinking) Under manual mode or under cutter radius compensation outside corner");               
            run.Add(8, "ReSET");
            run.Add(9, "(Not used)"); 
            run.Add(10, "(Not used)");    
            run.Add(11, "(Not used)");
            run.Add(12, "(Not used)");
            run.Add(13, "HPCC(during RISC operation)");
            // Status of emergency                          
            emergency = new Dictionary<short, string>();
            emergency.Add(0, "(Not emergency)");
            emergency.Add(1, "EMerGency");
            // Status of alarm
            alarm = new Dictionary<short, string>();
            alarm.Add(0, "(No alarm)");
            alarm.Add(1, "ALarM");
        }

        public static string getDesc(StatusType type, short code)
        {
            if (type == StatusType.RUN)
            {
                return run[code];
            }
            else if (type == StatusType.EMERGENCY)
            {
                return emergency[code];
            }
            else if (type == StatusType.ALARM)
            {
                return alarm[code];
            }
            else
